...
async def test_search_repositories_valid(monkeypatch):
    ...
    monkeypatch.setattr(core.github, "gh_search_repositories", lambda query: expected_result)
    # Act
...
async def test_get_file_contents_valid(monkeypatch):
    ...
    monkeypatch.setattr(core.github, "gh_get_file_contents", lambda owner, repo, path, branch=None: expected_content)
...
async def test_list_commits_valid(monkeypatch):
    ...
    monkeypatch.setattr(core.github, "gh_list_commits", lambda owner, repo, branch=None: expected_commits)
...
async def test_list_issues_valid(monkeypatch):
    ...
    monkeypatch.setattr(core.github, "gh_list_issues", lambda owner, repo, state="open", labels=None, sort="created", direction="desc": expected_issues)
...
async def test_get_repository_valid(monkeypatch):
    ...
    monkeypatch.setattr(core.github, "gh_get_repository", lambda owner, repo: expected_repo)
