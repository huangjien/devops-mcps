...
def jenkins_get_jobs() -> Union[List[Dict[str, Any]], Dict[str, str]]:
  ...
  if not jenkins_client.j:
    ...
    if (
      not os.environ.get("JENKINS_URL")
      or not os.environ.get("JENKINS_USER")
      or not os.environ.get("JENKINS_TOKEN")
    ):
  ...
  try:
    ...
    result = [
      _to_dict(jenkins_client.j[job_name]) for job_name in jobs
    ]  # modified to use .values()
...
def jenkins_get_all_views() -> Union[List[Dict[str, Any]], Dict[str, str]]:
  ...
  if not jenkins_client.j:
    ...
    return {"error": "Jenkins client not initialized. Set environment variables."}
  try:
    views = jenkins_client.j.views.keys()
    logger.debug(f"Found {len(views)} views.")
    result = [
      _to_dict(jenkins_client.j.views[view_name]) for view_name in views
    ]  # modified to use .values()
