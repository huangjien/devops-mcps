...
class MetricsCollector:
  ...
  def stop_timer(
    ...
    self.observe(f"{metric_name}_duration_ms", duration * 1000, tags)
    return duration
  ...
  def export_prometheus(self) -> str:
    ...
    for name, value in sorted(self.counters.items()):
      ...
      lines.append(f"devops_mcps_{name} {value}")
    ...
    for name, values in sorted(self.histograms.items()):
      if values:
        ...
        lines.append(f"devops_mcps_{name}_count {stats['count']}")
  ...
  def _format_metric_name(
    ...
    if tags:
      ...
      return f"{name}_{{{','.join(tag_parts)}}}"
    return name


# Global metrics collector instance
_metrics_enabled = os.environ.get("METRICS_ENABLED", "true").lower() == "true"
metrics = MetricsCollector(enabled=_metrics_enabled)
