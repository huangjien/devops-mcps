...
def validate_github_config() -> Tuple[bool, Optional[Dict[str, Any]]]:
...
def validate_jenkins_config() -> Tuple[bool, Optional[Dict[str, Any]]]:
  ...
  if log_length:
    try:
      log_length_int = int(log_length or "0")
      if log_length_int <= 0:
        return False, create_configuration_error(
...
def validate_artifactory_config() -> Tuple[bool, Optional[Dict[str, Any]]]:
...
def validate_azure_config() -> Tuple[bool, Optional[Dict[str, Any]]]:
...
def validate_server_config() -> Tuple[bool, Optional[Dict[str, Any]]]:
  """Validate server configuration.

  Returns:
      Tuple of (is_valid, error_dict or None)
  """
  port = os.environ.get("MCP_PORT") or "3721"
  log_level = os.environ.get("LOG_LEVEL") or "INFO"

  # Validate MCP_PORT
  try:
    port_int = int(port or "3721")
...
def validate_all_config(strict: bool = False) -> Tuple[bool, List[str]]:
  ...
  if not is_valid:
    if error:
      errors.append(str(error.get("error", "GitHub configuration error")))
    else:
      errors.append("GitHub configuration error")

  is_valid, error = validate_jenkins_config()
  if not is_valid:
    if error:
      errors.append(str(error.get("error", "Jenkins configuration error")))
    else:
      errors.append("Jenkins configuration error")

  is_valid, error = validate_artifactory_config()
  if not is_valid:
    if error:
      errors.append(str(error.get("error", "Artifactory configuration error")))
    else:
      errors.append("Artifactory configuration error")

  is_valid, error = validate_azure_config()
  if not is_valid:
    if error:
      errors.append(str(error.get("error", "Azure configuration error")))
    else:
      errors.append("Azure configuration error")

  is_valid, error = validate_server_config()
  if not is_valid:
    if error:
      errors.append(str(error.get("error", "Server configuration error")))
    else:
      errors.append("Server configuration error")
